"""Collect Asia market index data via yfinance."""
...
def _load_tickers() -> list[dict]:
    with open(SOURCES_PATH) as f:
        return json.load(f)["market_tickers"]["asia_indices"]
...
def _get_index_data(ticker_info: dict) -> dict:
    try:
        ...
        print(f"[asia_markets] Error fetching {ticker_info['ticker']}: {e}")
...
def collect() -> list[dict]:
    ...
    print(f"[asia_markets] Collected {len(results)} Asia indices")
    return results
